...
class Wdb(object):
    ...
    def set_trace(self, frame=None, break_=True):
        ...
        self.reset()
        trace = (
            self.trace_dispatch if trace_log.level >= 30 else self.trace_debug_dispatch
        )
            ...
            if len(obj) > 2:
                ...
                dict_repr += ", ".join(
                    [
                        ...
                        + ": "
                        + self.safe_better_repr(val, context, html, level, full)
                        for key, val in dict_sorted(obj.items(), key=lambda x: x[0])
                    ]
                )
            dict_repr += closer
            return dict_repr
    ...
    def interaction(
        ...
        log.info(
            "Interaction %r %r %r %r" % (frame, tb, exception, exception_description)
        )
